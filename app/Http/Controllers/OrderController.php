...
class OrderController extends  Controller{
    ...
    public function orderUpdate(Request $request){
        try {
            $order = Order::where('id', $request->order_id)->select('total_price', 'shipping_tax', 'shipping_price', 'user_id')->first();
            ...
            if(isset($request->product_id)){
                foreach ($request->product_id as $product_id) {
                    ...
                    if(isset($request->delete_id[$product_id])){

                        \DB::table('order_products')->where('id', $request->delete_id[$product_id])->delete();
        ...
        if($request->new_product_id1){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id1)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id1,
                    'quantity'    => $request->new_product_qty1,
                    'price'       => $request->product_price1,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty1*$request->product_price1;
                $product = Product::where('id', $request->new_product_id1)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty1*$request->product_price1;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id2){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id2)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id2,
                    'quantity'    => $request->new_product_qty2,
                    'price'       => $request->product_price2,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty2*$request->product_price2;
                $product = Product::where('id', $request->new_product_id2)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty2*$request->product_price2;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id3){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id3)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id3,
                    'quantity'    => $request->new_product_qty3,
                    'price'       => $request->product_price3,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty3*$request->product_price3;
                $product = Product::where('id', $request->new_product_id3)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty3*$request->product_price3;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id4){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id4)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id4,
                    'quantity'    => $request->new_product_qty4,
                    'price'       => $request->product_price4,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty4*$request->product_price4;
                $product = Product::where('id', $request->new_product_id4)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty4*$request->product_price4;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id5){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id5)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id5,
                    'quantity'    => $request->new_product_qty5,
                    'price'       => $request->product_price5,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty5*$request->product_price5;
                $product = Product::where('id', $request->new_product_id5)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty5*$request->product_price5;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id6){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id6)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id6,
                    'quantity'    => $request->new_product_qty6,
                    'price'       => $request->product_price6,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty6*$request->product_price6;
                $product = Product::where('id', $request->new_product_id6)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty6*$request->product_price6;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id7){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id7)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id7,
                    'quantity'    => $request->new_product_qty7,
                    'price'       => $request->product_price7,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty7*$request->product_price7;
                $product = Product::where('id', $request->new_product_id7)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty7*$request->product_price7;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id8){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id8)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id8,
                    'quantity'    => $request->new_product_qty8,
                    'price'       => $request->product_price8,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty8*$request->product_price8;
                $product = Product::where('id', $request->new_product_id8)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty8*$request->product_price8;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id9){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id9)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id9,
                    'quantity'    => $request->new_product_qty9,
                    'price'       => $request->product_price9,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty9*$request->product_price9;
                $product = Product::where('id', $request->new_product_id9)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty9*$request->product_price9;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id10){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id10)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id10,
                    'quantity'    => $request->new_product_qty10,
                    'price'       => $request->product_price10,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty10*$request->product_price10;
                $product = Product::where('id', $request->new_product_id10)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty10*$request->product_price10;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id11){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id11)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id11,
                    'quantity'    => $request->new_product_qty11,
                    'price'       => $request->product_price11,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty11*$request->product_price11;
                $product = Product::where('id', $request->new_product_id11)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty11*$request->product_price11;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id12){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id12)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id12,
                    'quantity'    => $request->new_product_qty12,
                    'price'       => $request->product_price12,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty12*$request->product_price12;
                $product = Product::where('id', $request->new_product_id12)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty12*$request->product_price12;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id13){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id13)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id13,
                    'quantity'    => $request->new_product_qty13,
                    'price'       => $request->product_price13,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty13*$request->product_price13;
                $product = Product::where('id', $request->new_product_id13)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty13*$request->product_price13;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id14){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id13)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id14,
                    'quantity'    => $request->new_product_qty14,
                    'price'       => $request->product_price14,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty14*$request->product_price14;
                $product = Product::where('id', $request->new_product_id14)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty14*$request->product_price14;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
        ...
        if($request->new_product_id15){
            $pro_ex = OrderProduct::where('product_id', $request->new_product_id15)->select('id')->first();
            if(empty($pro_ex)){
                OrderProduct::create([
                    'order_id'    => $request->order_id,
                    'product_id'  => $request->new_product_id15,
                    'quantity'    => $request->new_product_qty15,
                    'price'       => $request->product_price15,
                    'user_id'     => $order->user_id,
                ]);
                $new_pro_price += $request->new_product_qty15*$request->product_price15;
                $product = Product::where('id', $request->new_product_id15)->select('tax')->first();
                if($product){
                    if($product->tax == 1){
                        $p_price = $request->new_product_qty15*$request->product_price15;
                        $product_tax_price += ($p_price/100)*$product_tax;
                    }
                }
            }
    ...
    $total_price = $product_tax_price+$new_pro_price+$order->shipping_tax+$order->shipping_price; 
        ...
        $current_order = Order::where('id', $request->order_id)->first(); 
        $u_dt = User::where('id', $order ->user_id)->select('email')->first();
        $email = $u_dt->email;
        ...
        $subject = 'Vedyas - Order Update '. $current_order->order_nr.'';
        $subject1 = 'Vedyas - Order Update '. $current_order->order_nr.'';
        foreach ($emails as $key => $value) {
          $value = str_replace(' ', '', $value);
          \Mail::send('email.admin_order', $data, function($message) use ($email, $value, $subject1){
            $message->from($email);
            $message->to($value);
