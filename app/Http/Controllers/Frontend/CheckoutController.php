  ...
  public function shipping_calculation(Request $request){
      ...
      if(isset($cart_products)) { 
        foreach($cart_products as $cart_product) {
            ...
            $product_t = Product::where('id', $cart_product->pid)->select('tax')->first();

            if($product_t){
                if($product_t->tax == 1){
                    $p_price = $cart_product->quantity*$s_price;
    ...
    $total_price = $sale_price + $shipping_price + $shipping_tax_price + $product_tax_price;

    $data['shipping_price'] = $shipping_price;
    $data['shipping_type']  = $shipping_type;
    $data['shipping_tax_price']  = number_format($shipping_tax_price + $product_tax_price, 2);
...
$input['shipping_tax'] = $shipping_tax_price;

        $apiKey = config('F9802AWGyjDwF5mTv1vF4WQXnkpBiVJH0jZewv9Fdemb8aZq6+WnmqpkaWeif8AcqngZYQ');
        $apiPassword = config('Rituishi@1234');

        $client = Rapid::createClient($apiKey, $apiPassword, Rapid\Client::MODE_PRODUCTION); 
        // $rapidAPI = new \Eway\Rapid($apiKey, $apiPassword, \Eway\Rapid::MODE_PRODUCTION);
        ...
        // // Redirect user to Eway hosted payment page
        // dd($response->FormActionURL); 
        // return redirect($response->FormActionURL);
    ...
    $total_pay_amount1 =  $sale_price + $shipping_price + $product_tax_price + $shipping_tax_price;
    $total_pay_amount =  $total_pay_amount1;
    $input['order_from'] = "WEB";
    $input['user_id']     =  $user_id;
    $input['total_price'] = $total_pay_amount;
    $input['product_tax'] = $product_tax_price;
...
if(isset($cart_products)) {
  foreach($cart_products as $cart_product){
      ...
      $OrderProduct->save();
        ...
        $subject = 'Vedyas - Order Confirm '. $current_order->order_nr.'';
        $subject1 = 'Vedyas - New Order Received '. $current_order->order_nr.'';
