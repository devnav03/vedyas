    ...
    public function checkEmail(Request $request)
    {
        try{
            ...
            if(!empty($user_detail)) {
                ...
                // $user_id = route('update_password', $user_id);
                // $data['user_id'] = \encrypt($user_detail->id);
                // \Mail::send('email.forgot_password', $data, function($message) use($email){
                //     $message->from('no-reply@vedyas.com.au');
                //     $message->to($email);
                //     $message->subject('Vedyas - Forgot Password');
                // });
        ...
        $result = file_get_contents('https://sspl20.com/email-api/api/forgot-pasw-wishmed', false, $context);
