...
class Product extends Model
{
    ...
    public function getProduct($search = null, $skip, $perPage) {
        ...
        $img_filter = '';
        if(isset($search['featured_image'])){
            if($search['featured_image'] == 'With Images'){
                $img_filter = 0;
            } else {
       ...
       // dd($search);
        ...
        if (is_array($search) && count($search) > 0) {
            ...
            if($img_filter == 0){
                $f5 = (array_key_exists('featured_image', $search)) ? " AND (products.featured_image != 'no-image')" : ""; 
            }

            if($img_filter == 1){
                $f5 = (array_key_exists('featured_image', $search)) ? " AND (products.featured_image LIKE '%" .
                addslashes(trim($search['featured_image'])) . "%')" : "";
            }
